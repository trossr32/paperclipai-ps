function PClip-Secret-List {
    ...
    Invoke-PClipApi -Method GET -Path "/companies/$CompanyId/secrets" -BaseUrl $BaseUrl -Token $Token
}
...
function PClip-Secret-Create {
    ...
    Invoke-PClipApi -Method POST -Path "/companies/$CompanyId/secrets" -Body @{ name = $Name; value = $Value } -BaseUrl $BaseUrl -Token $Token
}
...
function PClip-Secret-Update {
    ...
    process {
        Invoke-PClipApi -Method PATCH -Path "/secrets/$SecretId" -Body @{ value = $Value } -BaseUrl $BaseUrl -Token $Token
    }
}
